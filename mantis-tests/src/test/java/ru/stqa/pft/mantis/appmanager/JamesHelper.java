...
public class JamesHelper {
    ...
    private void initTelnetSession() {
        ...
        try {
            ...
            in = telnet.getInputStream(); //чтобы читать данные
            out = new PrintStream( telnet.getOutputStream() ); //чтобы отправлять команды
    ...
    public List<MailMessage> waitForMail(String username, String password, long timeout) throws MessagingException {
        initTelnetSession();
        long now = System.currentTimeMillis(); //запоминает момент начала ожидания
    ...
    public List<MailMessage> getAllMail(String username, String password) throws MessagingException {
        Folder inbox = openInbox(username, password); //открыть почтовый ящик - требование POP3
        List<MailMessage> messages = Arrays.asList(inbox.getMessages()).stream().map((m) -> toModelMail(m))
                .collect(Collectors.toList());
        closeFolder(inbox); //закрыть почтовый ящик - требование POP3
        return messages;
    }
